            ...
            for i in range(order_n):
                ...
                if i == 0:
                    ...
                    if i + 1 < order_n:
                        ...
                        if order_dict[order_id].destination_id == order_dict[sub_order_id].origin_id:
                            transit_time = 0
                        elif (order_dict[order_id].destination_id,
                              order_dict[sub_order_id].origin_id) not in cost_dict.keys():
                            transit_time = INF
                        else:
...
def timePenalty(schedule_dict):
    """
    辅助函数，对不能按服务时间到达顾客的情况进行惩罚
    对给定路线，计算到达每个客户的时间
    索引给定的最迟到达时间
    :param schedule_dict: 记录每个顾客的时间相关信息的字典,key:order_id,value:OrderSchedule
    :return: time_delay
    """
    ...
    time_delay = 0
    cnt = 0
    for _, v in schedule_dict.items():
        time_diff = (v.actual_dropoff_time - v.expect_dropoff_time).total_seconds()
        time_delay += max(0, time_diff)
        if time_diff > 0:
            cnt += 1
    # print('超时订单数量：', cnt)
    return time_delay
...
def evaluate(ind, c1=150000.0, c2=1000):
    """评价函数，返回解码后路径的总长度，c1, c2分别为车辆超载与不能服从给定时间窗口的惩罚系数"""
    customers, routes = decodeInd(ind)[0],  decodeInd(ind)[1]  # 将个体解码为路线
...
def mutate(ind):
    ...
    """
    # 随机选择n个数，置为相反数；
    random_idx1 = random.sample(range(len(ind)), len(ind)//150)
    for i in random_idx1:
        ind[i] = - ind[i]

    # 随机选择m个数，重新生成；
    random_idx2 = random.sample(range(len(ind)), len(ind) // 50)
    for i in random_idx2:
        ind[i] = int(ind[i]) + random.random()

    # 随机选择k个数，重新选择车型
    random_idx3 = random.sample(range(len(ind)), len(ind) // 80)
    for i in random_idx3:
        f_part = ind[i] - int(ind[i])
        ind[i] = f_part + random.randint(0, 4)
...
customers, routes = decodeInd(bestInd)[0], decodeInd(bestInd)[1]
...
print('各辆车上负载为：')
print(cal_route_load(customers))
print('等待时间')
schedule_dict = calcRouteServiceTime(customers, routes)
print(cal_route_waitting_time(schedule_dict))
